...
public class AccommodationServiceApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		MakeNewReservationResponse r = (MakeNewReservationResponse) rabbitTemplate.convertSendAndReceive("reservation-make-queue", new MakeNewReservationCommand(reservationToCancelUuid, LocalDateTime.now(), LocalDate.now(), LocalDate.now(), "type1", 1, 2, 0, testAddedHotelUuid));
		...
		x = myObj.nextLine();
		//
		rabbitTemplate.convertSendAndReceive("reservation-make-queue", new MakeNewReservationCommand(UUID.randomUUID(), LocalDateTime.now(), LocalDate.now(), LocalDate.now(), "type1", 1, 2, 0, testAddedHotelUuid));
		x = myObj.nextLine();
		rabbitTemplate.convertSendAndReceive("reservation-make-queue", new MakeNewReservationCommand(UUID.randomUUID(), LocalDateTime.now(), LocalDate.of(2000,1,1), LocalDate.of(2000,1,10), "type1", 1, 2, 0, testAddedHotelUuid));
		x = myObj.nextLine();
		MakeNewReservationResponse r2 = (MakeNewReservationResponse) rabbitTemplate.convertSendAndReceive("reservation-make-queue", new MakeNewReservationCommand(UUID.randomUUID(), LocalDateTime.now(), LocalDate.of(2000,1,1), LocalDate.of(2000,1,10), "type1", 1, 2, 0, testAddedHotelUuid));
